...
## Modeling artifacts archived

Note: modeling code and trained-model artifacts have been archived out of the main data-prep tree to keep this repository focused on preparation. You can find all modeling/training scripts and baseline artifacts under the top-level folder `archive_modeling/` (e.g. `archive_modeling/extract_features_and_baseline.py`, `archive_modeling/modeling/train_baseline.py`, and `archive_modeling/baseline/`).

This keeps `data/`, `notebooks/`, and the data-preparation utilities under `tools/` as the canonical data-prep deliverable. If you want the modeling code removed entirely from the repository (or moved to a separate repo), I can help package `archive_modeling/` into a release artifact or move it to a different branch.
